  ...
  module RunnerHelpers # :nodoc:
    class Session < Mack::RunnerHelpers::Base
      ...
      def start(request, response, cookies)
        if app_config.mack.use_sessions
          ...
          unless self.sess_id
            ...
            sess = Mack::SessionStore.store.get(self.sess_id, request, response, cookies)
            if sess
      ...
      def complete(request, response, cookies)
        ...
        Mack::SessionStore.store.set(request.session.id, request, response, cookies) if app_config.mack.use_sessions
      end
      ...
      def create_new_session(request, response, cookies)
        ...
        Mack::SessionStore.store.set(request.session.id, request, response, cookies)
        id
      end
