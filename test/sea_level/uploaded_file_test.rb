...
class ControllerBaseTest < Test::Unit::TestCase
  ...
  def setup
    FileUtils.cp(File.join(MACK_ROOT, "public", "images", "logo.gif"), File.join(MACK_ROOT, "public", "something", "logo.gif"))
    @uploaded_file = Mack::Request::UploadedFile.new(:type => "image/gif", :filename => "logo.gif", :head => "Content-Disposition: form-data; name=\"my_new_file\"; filename=\"logo.gif\"\r\nContent-Type: image/gif\r\n", :tempfile => File.open(File.join(MACK_ROOT, "public", "something", "logo.gif")), :name=>"my_new_file")
  end                                                                                
  
  def teardown
    FileUtils.rm_rf(File.join(MACK_ROOT, "tmp"))
  end
  
  def test_save_to
    @uploaded_file.save_to(File.join(MACK_ROOT, "tmp", @uploaded_file.file_name))
    assert File.exists?(File.join(MACK_ROOT, "tmp", @uploaded_file.file_name))
    assert !File.exists?(@uploaded_file.temp_file.path)    
  end
  
  def test_save_to_array
    @uploaded_file.save_to([MACK_ROOT, "tmp", @uploaded_file.file_name])
    assert File.exists?(File.join(MACK_ROOT, "tmp", @uploaded_file.file_name))
    assert !File.exists?(@uploaded_file.temp_file.path)    
  end
  ...
  def test_save
    path = File.join(MACK_ROOT, "tmp", @uploaded_file.file_name)
